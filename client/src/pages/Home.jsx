...
const Home = () => {
  ...
  const getTotalBitcoinTransferred = (details) => {
    if (!details) return 0;
    return details.out.reduce((total, output) => total + output.value, 0) / 100000000;
  }
  ...
  return (
    <>
      <main className='bg-gray-950 min-h-screen font-sans'>
        ...
        <section>
          <h2 className='text-7xl text-white w-2/3 leading-tight tracking-tight mx-auto p-12'>Analyse & Track Crypto Currencies transactions back to Source</h2>
        </section>
        ...
        {txDetails && (
          <section className='p-4 text-white'>
            ...
            <div className='m-2 justify-center items-center flex flex-col'>
              <h3 className='text-3xl p-2 text-sky-500 flex flex-row items-center gap-3'>Senders Wallet Address / Inputs: <HugeiconsMoneySendCircle className="text-4xl" /></h3>
            ...
            <div className='m-2 flex flex-col justify-center
               items-center'>
              <h3 className='text-3xl p-2 text-sky-500 flex flex-row gap-3 items-center' > Recievers Wallet Address / Outputs:  <HugeiconsMoneyReceiveCircle className="text-4xl" /></h3 >
              <ul>
                {txDetails.out.map((output, index) => (
                  <li key={index} className='leading-loose text-lg p-2 text-center'>
                    {output.addr}
            ...
            </div>            <Card Label={"Total Bitcoin Transferred"} Value={getTotalBitcoinTransferred(txDetails)} />


            <div className='flex justify-center items-center'>
              <IonTime className="text-6xl text-slate-400 m-4" />
            </div>
            <Card Label={'Time Confirmed (UTC)'} Value={new Date(txDetails.time * 1000).toLocaleString()} />
          </section>
        )}
      </main>
    </>
